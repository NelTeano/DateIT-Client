...
export default function TinderCards() {
  ...
  const removeCard = (isLove) => {
    setCards(prev => prev.slice(1));
    setDragOffset({ x: 0, y: 0 });
    setStatus(null);
  };

  const handleButtonClick = (isLove) => {
        ...
        <div className="relative w-[90vw] max-w-md h-[70vh]">
          {cards.map((card, index) => (
            ...
            >
              ...
              />
              <div className="p-4">
                <h3 className="text-2xl font-bold mb-4 pointer-events-none">
                  {card.title}
                </h3>
                <p className="text-lg text-gray-600 pointer-events-none">
                  {card.description}
                </p>
              </div>
            </div>
